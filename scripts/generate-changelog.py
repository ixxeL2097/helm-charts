...
def format_author(name: str, email: str) -> str:
    ...
    If the name looks like a GitHub username (no spaces), use @username.
    If the email contains a GitHub noreply, extract the username from it.
    Otherwise, display the name without @ to avoid broken links.
    """
    ...
    # Fallback: display name without @ to avoid broken GitHub links
    return name
...
def format_commit_message(commit: git.Commit, repo_url: str) -> str:
    ...
    author = format_author(commit.author.name, commit.author.email)
